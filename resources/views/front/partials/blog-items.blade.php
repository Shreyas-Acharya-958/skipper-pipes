@forelse($blogs as $blog)
    <div class="col-lg-4 col-md-6 single-item">
        <div class="item">
            <div class="thumb">
                <a title="{{ $blog->title }}" href="{{ route('front.blogs.show', $blog->slug) }}"><img src="{{ asset('storage/' . $blog->image_1) }}" alt="{{ image_alt_text('storage/' . $blog->image_1, $blog->title) }}"></a>
            </div>
            <div class="info">
                <div class="meta">
                    <ul>
                ...
                <div class="content">
                    <h4><a href="{{ route('front.blogs.show', $blog->slug) }}">{{ $blog->title }}</a></h4>
                    <p> {{ Str::limit($blog->short_description, 150) }} </p>
                    <a class="more-btn" href="{{ route('front.blogs.show', $blog->slug) }}">Read More <i class="fas fa-plus"></i></a>
                </div>
            </div>
        </div>
    </div>
