                ...
                @foreach ($banners as $key => $banner)
                    <div class="carousel-item {{ $key == 0 ? 'active' : '' }}" data-interval="3000">
                        <img src="{{ asset('storage/' . $banner->mobile_image) }}" class="d-block w-100"
                            alt="{{ image_alt_text('storage/' . $banner->mobile_image, $banner->title) }}" {{ $key != 0 ? 'loading=lazy' : 'fetchpriority=high' }} >
                    </div>
                @endforeach
            </div>
                ...
                <div class="row">
                    @foreach ($categories as $category)
                        <div class="col-lg-4 col-md-6 product-category-col" data-aos="fade-up" data-aos-duration="1000"
                            data-aos-delay="100">
                            <a href="{{ $category->products()->where('title', $category->name)->exists() ? route('front.products.show', ['slug' => $category->slug]) : route('front.products.show', ['slug' => ($category?->products()->orderBy('title','asc')->first()->slug ?? '#') ]) }}">
    ...
    @if ($sectionThree && $sectionThree->title)
        <section class="trusted-one">
            <div class="trusted-one__bg jarallax" data-jarallax data-speed="0.2" data-imgPosition="50% 0%"
                style='background-image: url("{{asset('assets/img/Skipper-pipes-homepage-video-banner.webp')}}");'>
            </div>
            <div class="container">
