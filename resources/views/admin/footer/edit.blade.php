    ...
    <div class="container-fluid" style="padding: 20px;">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="mb-0">Footer Sections</h4>
            </div>
            <div class="card-body">
                <form id="footerForm" action="{{ route('admin.footer.update') }}" method="POST">
                    @csrf
                    @method('POST')
                    <div class="form-group mb-3">
                        <label for="section1" class="form-label">Section 1 Content</label>
                        <textarea class="form-control tinymce" id="section1" name="section1" rows="6">{{ old('section1', $footer->section1 ?? '') }}</textarea>
                    </div>
                    <div class="form-group mb-3">
                        <label for="section2" class="form-label">Section 2 Content</label>
                        <textarea class="form-control tinymce" id="section2" name="section2" rows="6">{{ old('section2', $footer->section2 ?? '') }}</textarea>
                    </div>
                    <div class="form-group mb-3">
                        <label for="section3" class="form-label">Section 3 Content</label>
                        <textarea class="form-control tinymce" id="section3" name="section3" rows="6">{{ old('section3', $footer->section3 ?? '') }}</textarea>
                    </div>
                    <div class="form-group mb-3">
                        <label for="section4" class="form-label">Section 4 Content</label>
                        <textarea class="form-control tinymce" id="section4" name="section4" rows="6">{{ old('section4', $footer->section4 ?? '') }}</textarea>
                    </div>
