    ...
    <div class="container-fluid" style="padding: 20px;">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="mb-0">Media Sections</h4>
                <a href="{{ url('/admin/media') }}" class="btn btn-secondary">
                    <i class="fas fa-arrow-left"></i> Back to Pages
                </a>
            </div>
            <div class="card-body">
                <!-- Nav tabs -->
                <ul class="nav nav-tabs mb-3" id="mediaSectionTabs" role="tablist">
                ...
                <!-- Tab content -->
                <div class="tab-content" id="mediaSectionTabsContent">
                    <div class="tab-pane fade p-3" id="head" role="tabpanel">
                        <div>
                            <span class="fw-bold">Information:</span> Media Head Section
                        </div>
                        <form id="headForm" class="section-form" action="{{ route('admin.media.section1.save') }}"
                            method="POST">
                            @csrf
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" class="form-control" id="head_title" name="title"
                                    value="{{ $mediaSectionOne->title ?? '' }}" readonly>
                            </div>
                            <div class="form-group mb-3">
                                <label for="head_description" class="form-label">Description</label>
                                <textarea class="form-control tinymce" id="head_description" name="description" rows="6" readonly>{{ $mediaSectionOne->description ?? '' }}</textarea>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="tab-pane fade show active p-3" id="section2" role="tabpanel">
                        <div>
                            <span class="fw-bold">Information:</span> Media Main Section
                        </div>
                        <form id="section2Form" class="section-form" action="{{ route('admin.media.section2.save') }}"
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" class="form-control" id="section2_title" name="title"
                                    value="{{ $mediaSectionTwo->title ?? '' }}" readonly>
                            </div>
                            <div class="form-group mb-3">
                                <label class="form-label">Image</label>
                                <div class="mb-2">
                                    @if (isset($mediaSectionTwo) && $mediaSectionTwo->image)
                            ...
                            <div class="form-group mb-3">
                                ...
                                <textarea class="form-control tinymce" id="section2_description" name="description" rows="6" readonly>{{ $mediaSectionTwo->description ?? '' }}</textarea>
                            </div>
                        </form>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            if (!activeTab) {
                activeTab = localStorage.getItem('activeMediaSectionTab') || '#section1';
            }
            ...
            $('#mediaSectionTabs button[data-bs-toggle="tab"]').on('shown.bs.tab', function(e) {
                const targetTab = $(e.target).data('bs-target');
                localStorage.setItem('activeMediaSectionTab', targetTab);
            ...
            $('.section-form').on('submit', function() {
                ...
                // Update active tab
                const activeTab = localStorage.getItem('activeMediaSectionTab') || '#section1';
